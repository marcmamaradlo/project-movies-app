...
const SearchContent = (props) => {
  ...
  const filterMenu = context.filterMenu;
  ...
  async function getMovieDataWithPagination() {
    ...
    try {
      const movieResult = await axios.get(
        `https://api.themoviedb.org/3/search/movie?query=${userInput}&api_key=${apiKey}&page=${currentPage}`
      );
  ...
  async function getTrendingData() {
    ...
    try {
      const movieResult = await axios.get(
        `https://api.themoviedb.org/3/search/movie?query=${userInput}&api_key=${apiKey}&page=1`
      );
      setMovieResult(movieResult.data);
      // console.log(movieResult.data);

      const tvResult = await axios.get(
        `https://api.themoviedb.org/3/search/tv?query=${userInput}&api_key=${apiKey}&page=1`
      );
      setTvResult(tvResult.data);
      // console.log(tvResult.data);

      const personResult = await axios.get(
        `https://api.themoviedb.org/3/search/person?query=${userInput}&language=en-US&page=1&api_key=${apiKey}`
      );
      setPersonResult(personResult.data);
      // console.log(personResult.data);

      const collectionResult = await axios.get(
        `https://api.themoviedb.org/3/search/collection?query=${userInput}&language=en-US&page=1&api_key=${apiKey}`
      );
