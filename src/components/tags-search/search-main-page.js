  ...
  async function getTrendingData() {
    ...
    try {
      const movieData = await axios.get(
        `https://api.themoviedb.org/3/trending/movie/${movieSelector}?api_key=${apiKey}`
      );
      setDataTrendingMovie(movieData.data.results);
      const tvData = await axios.get(
        `https://api.themoviedb.org/3/trending/tv/${tvSelector}?api_key=${apiKey}`
      );
      setDataTrendingTV(tvData.data.results);
      const personData = await axios.get(
        `https://api.themoviedb.org/3/trending/person/${personSelector}?api_key=${apiKey}`
      );
    ...
    <>
      ...
      <div className="container bg-dark">
        <Trending
          ...
          dataType="movie"
          action={setMovieSelector}
          selector={movieSelector}
        />
        <Trending
          data={dataTrendingTV}
          dataType="tv"
          action={setTvSelector}
          selector={tvSelector}
        />
        <Trending
          data={dataTrendingPerson}
          dataType="person"
          action={setPersonSelector}
