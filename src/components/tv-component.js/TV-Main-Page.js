  ...
  async function getDynamicPageData() {
    try {
      ...
      const dataDetailes = await axios.get(
        `https://api.themoviedb.org/3/${selector}/${params.id}?api_key=${apiKey}&append_to_response=credits`
      );
      setDynamicData(dataDetailes.data);
      const dataKeywords = await axios.get(
        `https://api.themoviedb.org/3/${selector}/${params.id}/keywords?api_key=${apiKey}`
      );
      setDynamicKeywords(dataKeywords.data.results);
      const similar = await axios.get(
        `https://api.themoviedb.org/3/${selector}/${params.id}/similar?api_key=${apiKey}&page=1`
      );
