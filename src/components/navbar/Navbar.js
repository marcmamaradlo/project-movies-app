  ...
  async function getSearchResults() {
    try {
      ...
      const response = await axios.get(
        `https://api.themoviedb.org/3/search/multi?query=${query}&language=en-US&page=1&api_key=${apiKey}`
      );
      ...
      <div className="section-zero-mp">
        <div className="banner-navbar">
          <div className="banner-navbar-logo">
            <p className="banner-navbar-menu">
          ...
          <div className="banner-navbar-menu">
            <p
              ...
              className={
                activePage === "movie"
                  ? "banner-navbar-menu-active"
                  : "banner-navbar-menu-default"
              }
            >
              MOVIES
            </p>
          ...
          <div className="burger-icon text-shadow">
            <i
              ...
              className={hamburgerIcon ? "display-none" : "fa-solid fa-bars"}
            ></i>
            <div className={hamburgerIcon ? "off-canvas-menu" : "display-none"}>
              <Link to="/" name="home" onClick={handleNavbarMenuButtons}>
                HOME
              </Link>
              <Link to="/" name="movie" onClick={handleNavbarMenuButtons}>
                MOVIES
              </Link>
              <Link to="/" name="tv" onClick={handleNavbarMenuButtons}>
                TV SHOW
              </Link>
              <Link to="/" name="person" onClick={handleNavbarMenuButtons}>
                PEOPLE
              </Link>
              <Link
                to="/search"
                name="search"
                onClick={handleNavbarMenuButtons}
              >
                SEARCH
              </Link>
              <i
